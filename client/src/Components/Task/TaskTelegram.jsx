...
const TaskTelegram = ({ showModal, setShowModal }) => {
  ...
  const handleTaskLinkClick = () => {
    window.open("https://t.me/tapmianus");
  ...
  const handleVerify = async () => {
    ...
    const response = await fetch(
      `https://api.telegram.org/bot${import.meta.env.VITE_TELE_TOKEN}/getChatMember?chat_id=-1002392942354&user_id=${id}`
    );
  ...
  return (
    <>
      {showModal ? (
        <div className="fixed z-50 slackey-regular left-0 right-0 top-0 bottom-0 flex justify-center taskbg px-[16px] h-full">
          <div className={`w-full flex flex-col items-center justify-start`}>
            ...
            <div className="flex flex-col w-full">
              <h1 className="text-[20px] font-semibold">Join the Tapmianus Telegram Channel</h1>
              <p className="text-[#9a96a6] text-[16px] font-medium pt-1 pb-10">
                shleed my shneedenfeeden blafogniusman wastasm blkblk hoohoo
              </p>

              <p className="w-full text-center text-[14px] font-semibold text-[#49ee49] pb-4">
                {taskCompleted ? "Task is Completed" : ""}
              </p>
              <div className="bg-cards rounded-[10px] px-[14px] py-[5px] flex justify-between items-center">
                <div className="flex items-center flex-1 space-x-2 text-sm">
                  <div className="">
                    <img src={coinsmall} className="w-[50px]" alt="Coin Icon" />
                  </div>
                  <div className="flex flex-col space-y-1">
                    <span className="font-semibold slackey-regular">Reward?</span>
                    <div className="flex items-center">
                      <span className="font-medium slackey-regular">50K thats it</span>
                    </div>
                  </div>
                </div>
              </div>
              <h1 className="text-[20px] font-semibold pt-5 pb-3 px-2">
                Do it
              </h1>
              <div className="bg-cards rounded-[10px] p-[14px] flex justify-between items-center">
                <div className="flex items-center flex-1 space-x-2 text-sm">
                  <div className="flex flex-col space-y-1">
                    <span className="font-semibold slackey-regular">
